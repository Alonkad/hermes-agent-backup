...
- **Timezone Management (Israel Specific):** The server typically runs on UTC. When a user requests a reminder in Israel time (IST/IDT), you must manually calculate the UTC equivalent. Israel is UTC+3 (Summer) or UTC+2 (Winter).
  - Example: User asks for "every 8 hours starting at 23:00 Israel".
  - Calculation (Summer): 23:00 IST = 20:00 UTC. Every 8 hours from 20:00 UTC -> 04:00, 12:00, 20:00.
  - Cron schedule: `0 4,12,20 * * *`.
  - Always explain the conversion to the user to avoid confusion.
- **RTL Formatting in Hebrew Notifications (WhatsApp/Telegram):** When drafting reminder prompts in Hebrew, start the text with a Right-to-Left Mark (RLM, `\u200f`) to ensure correct alignment and readability in RTL-aware applications. This is especially important when the message contains mixed LTR/RTL characters (like dates or English nouns). If the user notes that the RLM symbol is visible and distracting, respect their preference to remove it.
